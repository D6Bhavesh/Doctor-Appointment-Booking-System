...
export const Appointment = () => {
  ...
  const getAvailableSlots = async ()=>{
    ...
    for(let i=0; i<7; i++){
      ...
      //get endtime of date with index
      ...
      while(currentDate < endTime){
        ...
        console.log("fuck" ,docInfo.slots_booked)
    ...
    <div>
      ...
      <div className='sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700'>
        ...
        <div className='flex items-center w-full gap-3 overflow-x-scroll mt-4'>
          {console.log("this slots",docSlots[slotIndex])}
          {
          docSlots.length && docSlots[slotIndex].map((item, index)=>(
            <p onClick={()=> setSlotTime(item.time)} key={index} className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${item.time === slotTime ? 'bg-primary text-white' : 'text-gray-300 border border-gray-300'}`}>{item.time.toLowerCase()}</p>
          ))}
        </div>
        <button onClick={bookAppointment} className='bg-primary text-white text-sm font-light px-14 py-3 rounded-full my-6 cursor-pointer'>Book Appointment</button>
      </div>
